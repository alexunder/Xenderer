...
int screen_init(int argc, char * argv[], int w, int h, const char *title);	// 屏幕初始化
...
gboolean on_key_event(GtkWidget *widget, GdkEventKey *event, gpointer data) {
	switch (event->keyval) {
		case GDK_KEY_Up:
			pos -= 0.01f;
			break;
...
// 初始化窗口并设置标题
int screen_init(int argc, char * argv[], int w, int h, const char *title) {
#ifdef WINDOWS
	...
	screen_h = h;
	screen_pitch = w * 4;
	screen_fb = (unsigned char *)malloc(screen_w * screen_h * 4);
	gtk_init(&argc, &argv);
#endif
